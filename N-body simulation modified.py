...
class Particle():
    ...
    def __init__(self,initial_pos,initial_vel, mass):
        ...
        self.m = mass
        self.G = 1  # change this to 6.67408 × 1e-11 if you want real world measuring units. before 1
    ...
    def calculate_acc(self,particles):
        for j in range(len(particles)):
            if self.i !=j:
                ...
                dx = self.pos.x - particles[j].pos.x
                dy = self.pos.y - particles[j].pos.y

                dist = (dx*dx + dy*dy + eps*eps)**(3/2)

                self.acc += (
                    (particles[j].pos - self.pos)
                    * particles[j].m
                    * self.G
                    * (1/dist)
                )
...
for i in range(20):
    par.append(Particle(initial_pos = Vec2(np.random.uniform(-100,100),np.random.uniform(-100,100)), initial_vel = Vec2(np.random.uniform(-1,1),np.random.uniform(-1,1)) , mass = np.random.uniform(0.01,5)))
...
duration = end - start
print("Simulation completed in " + str(duration) + " seconds.")    
...
fig, ax = plt.subplots(figsize=(19.2, 10.8)) # changed from 7, 7 to 19.2, 10.8 and figure to subplots
...
# ax.axis('equal')
# ax.axis([-100, 100, -100, 100]) # changed from -1, 30, -1, 30 to -100, 100, -100, 100 for a bigger field of view
